...
- **AI 自然语言搜索**：输入 `上周的报账表格->ai`，回车后由 OpenAI 兼容 API（已配 DeepSeek）转结构化查询在本地索引执行
...
- MSVC 工具链：`C:\Program Files (x86)\Microsoft Visual Studio\2022\BuildTools\VC\Tools\MSVC\14.44.35207`
- Windows SDK 10.0.26100：`D:\WinSDK-26100\sdk`（从 NuGet 官方包 `Microsoft.Windows.SDK.CPP*` 解压，注册表 `KitsRoot10` 已指向）

Git Bash 下构建需把 MSVC bin 放 PATH 最前（避免 `/usr/bin/link` 抢占）：

```bash
export PATH="/d/WinSDK-26100/x-cpp/c/bin/10.0.26100.0/x64:/c/Program Files (x86)/Microsoft Visual Studio/2022/BuildTools/VC/Tools/MSVC/14.44.35207/bin/Hostx64/x64:/c/Users/Administrator/.cargo/bin:$PATH"
```
...
数据目录：`%LOCALAPPDATA%\MonikaSearch\`（旧 EverythingPlus 目录启动时自动迁移）。`config.json` 存 AI 配置（endpoint/key/model，key 也可用环境变量 `MONIKA_AI_KEY` / `DEEPSEEK_API_KEY` 覆盖，凭据不进源码）。索引缓存 `index.bin` 约 170MB / 318 万文件。实测数据（本机，3 卷 336 万文件）：全量扫描 23s（release），缓存加载 <2s，单次搜索 <40ms，USN 增量实时捕获。
...
"mcp": { "servers": { "monikasearch": {
  "command": "D:\\Users\\Administrator\\Desktop\\everythingplus\\target\\release\\ep-mcp.exe",
  "args": []
} } }
```
...
- [ ] 内存占用优化（arena 存储 / 路径压缩；当前 318 万文件约 1.1GB）
